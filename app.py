...
import pandas
from chatbotmain import get_response, get_intent
import time
...
def human():
    if request.method =="POST":
        ...
        with sqlite3.connect('static/database.db') as conn:
            cur = conn.cursor()
            cur.execute(
                'INSERT INTO Consult (username, name, gender, content, topic, sentiment,date) VALUES (?, ?, ?, ?, ?, ?,?)',
                (username, name, gender, content, topic, sentiment,current_date))
            conn.commit()
...
def get_chatbot_response():
    ...
    with sqlite3.connect('static/database.db') as conn:
        ...
        cur.execute('INSERT INTO AI (username, content, date, topic,response,sentiment) VALUES (?, ?, ?, ?,?,?)',
                    (username, user_input, current_date, topic,response, sentiment ))
        conn.commit()
    # print(response)
